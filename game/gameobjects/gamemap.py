...
class GameMap:
    ...
    def __init__(self, renderer: pyasge.Renderer, tmx_file: str) -> None:
        ...
        self.tile_sz = 2  # Variable to scale the map
        if tmx_file == "./data/map/beach.tmx":
            self.tile_sz = 64 / 24
        if tmx_file == "./data/map/Special/Special_mk2.tmx":
            self.tile_sz = 2
        self.tile_size = [int(tmxdata.tilewidth * self.tile_sz), int(tmxdata.tileheight * self.tile_sz)]

        self.out_of_bounds = []
        for rect in tmxdata.layernames["NoGo"]:
            self.out_of_bounds.append([rect.x * self.tile_sz, rect.y * self.tile_sz,
                                       rect.width * self.tile_sz, rect.height * self.tile_sz])

        self.exit_zones = []
        for rect in tmxdata.layernames["ExitZone"]:
            self.exit_zones.append([rect.x * self.tile_sz, rect.y * self.tile_sz,
                                    rect.width * self.tile_sz, rect.height * self.tile_sz, rect.name])

        self.spawns = []
        if tmx_file == "./data/map/Special/Special_mk2.tmx":
            for pos in tmxdata.layernames["spawn_caps"]:
                self.spawns.append([pos.x * self.tile_sz, pos.y * self.tile_sz])
